export class HudController{
 ...
 combat(active){this.$('bossHud').style.display=active?'block':'none';this.$('shoot').style.display=active?'block':'none';this.$('lock').style.display=active?'block':'none';this.$('crosshair').style.display=active?'block':'none';if(!active){this.$('lock').classList.remove('active');this.$('lock').innerHTML='🔒<br>FIJAR';}}
